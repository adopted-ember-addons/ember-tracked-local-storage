...
module('Unit | Utility | tracked in local storage decorator', function (hooks) {
  ...
  test('defaultValue syncWithLocalStorage works', function (assert) {
    ...
    assert.strictEqual(
      service.syncedProp,
      testDefaultValue,
      'When LS has nothing, defaultValue is used',
    );
    ...
    assert.strictEqual(
      service.syncedProp,
      valueFromLS,
      'Value from LS is used when available',
    );
  });
  ...
  test('setting the macro will save the value to localStorage', function (assert) {
    let service = registerService.call(this);
    assert.notOk(
      trackedLocalStorage.getItem(testKeyName),
      'There is no value in LS yet',
    );
    ...
    assert.strictEqual(
      trackedLocalStorage.getItem(testKeyName),
      newValue,
      'The value in LS has been updated',
    );
    assert.strictEqual(
      service.syncedProp,
      newValue,
      'The getter also returns new value now',
    );
  });
  ...
  test('normalization works in the macro', function (assert) {
    ...
    assert.notOk(
      trackedLocalStorage.getItem(booleanKey),
      'There is no value in LS yet',
    );
    trackedLocalStorage.setItem(booleanKey, true);
    assert.true(
      service.syncedProp,
      'The value is normalized from string to boolean',
    );
  });
});
